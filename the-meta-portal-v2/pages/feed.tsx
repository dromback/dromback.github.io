...
import Recommendations from "../components/Metaverse/Recommendations";
import PageContentLayout from "../components/Layout/PageContent";
import PostLoader from "../components/Posts/Loader";
import PostItem from "../components/Posts/PostItem";
import { auth, firestore } from "../firebase/clientApp";
import usePosts from "../hooks/usePosts";
import Premium from "../components/Metaverse/Premium";
import PersonalHome from "../components/Metaverse/PersonalHome";
...
const Home: NextPage = () => {
  ...
  const getUserHomePosts = async () => {
    ...
    try {
      ...
      if (metaverseStateValue.mySnippets.length) {
        console.log("GETTING POSTS IN USER METAVERSES");

        const myMetaverseIds = metaverseStateValue.mySnippets.map(
          (snippet) => snippet.metaverseId
        );
        // Getting 2 posts from 3 communities that user has joined
        let postPromises: Array<Promise<QuerySnapshot<DocumentData>>> = [];
        [0, 1, 2].forEach((index) => {
          if (!myMetaverseIds[index]) return;

          postPromises.push(
            getDocs(
              query(
                collection(firestore, "posts"),
                where("metaverseId", "==", myMetaverseIds[index]),
                limit(3)
              )
            )
          );
      ...
      // User has not joined any metaverses yet
      else {
        console.log("USER HAS NO METAVERSES - GETTING GENERAL POSTS");
  ...
  }, [user, metaverseStateValue.initSnippetsFetched]);
